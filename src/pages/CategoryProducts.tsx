  ...
  const fetchCategoryProducts = async () => {
    try {
      ...
      const {
        ...
        error: categoryError
      } = await supabase.from('category_products').select('product_id').eq('category_id', categoryId).order('display_order', {
        ascending: true
      });
      ...
      const {
        data: productsData,
        error: productsError
      } = await supabase.from('krolist_products').select('*').in('id', productIds);
