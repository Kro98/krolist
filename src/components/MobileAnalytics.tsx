...
import { TrendingDown, TrendingUp, DollarSign, Target, Eye, Store, Tag, Clock, BarChart3, Maximize2, ChevronRight } from "lucide-react";
...
const analyticsData = {
  ...
  pricePatterns: [{
    pattern: "Weekly drops on Sunday",
    frequency: 67,
    avgSavings: 8.2
  }, {
    pattern: "End of month sales",
    frequency: 45,
    avgSavings: 15.5
  }, {
    pattern: "Holiday season drops",
    frequency: 89,
    avgSavings: 22.3
  }, {
    pattern: "Flash sales",
...
const CompactAnalyticsCard = ({
  ...
  return <div className="bg-gradient-card rounded-lg p-3 border min-h-[100px] w-full max-w-full overflow-hidden px-[5px]">
      ...
      {change && <div className={`${fontSizes[fontSize as keyof typeof fontSizes]} text-price-decrease truncate`}>{change}</div>}
    </div>;
};
...
const MiniChart = ({
  ...
  if (type === "progress") {
    return <div className="space-y-1.5 my-0 py-0 px-0 mx-0">
        {data.slice(0, 3).map((item, index) => <div key={index} className="space-y-1 mx-[50px]">
  ...
  return <div className="flex items-end justify-between h-10 gap-1 mx-[50px]">
      {data.slice(0, 3).map((item, index) => <div key={index} className="flex flex-col items-center flex-1">
          <div className="w-full bg-primary/20 rounded-t" style={{
...
export function MobileAnalytics() {
  ...
  if (!isMobile) {
    ...
    return <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {quickStats.map((stat, index) => <Card key={index} className="shadow-card">
                ...
                {analyticsData.priceChanges.map((period, index) => <div key={index} className="space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <div className="bg-price-increase/20 h-2 rounded-full">
                        <div className="bg-price-increase h-full rounded-full" style={{
                      ...
                      <div className="bg-price-decrease/20 h-2 rounded-full">
                        <div className="bg-price-decrease h-full rounded-full" style={{
                      width: `${period.decreases / (period.increases + period.decreases + period.stable) * 100}%`
                    }} />
                      </div>
                      <div className="bg-muted h-2 rounded-full">
                        <div className="bg-muted-foreground h-full rounded-full" style={{
  ...
  return <div className="space-y-4 max-w-sm mx-0 px-0">
      {/* Quick Stats Carousel - Mobile */}
      <div className="px-1">
        <Carousel className="w-full max-w-full">
          <CarouselContent className="-ml-1">
            {quickStats.map((stat, index) => <CarouselItem key={index} className="pl-1 basis-1/2">
      ...
      <div className="space-y-3 px-1">
        ...
        <div className="grid grid-cols-1 gap-3 mx-0">
          {sections.map(section => <Sheet key={section.id}>
              <SheetTrigger asChild>
                <Card className="shadow-card hover:shadow-hover transition-all duration-200 cursor-pointer bg-gradient-card min-h-[140px] w-full max-w-full mx-0">
                  <CardContent className="p-3 h-full flex flex-col justify-between py-[12px] mx-0">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground leading-tight truncate py-0">{t(`analytics.${section.id}Desc`)}</p>
                      </div>
                    </div>
                    
                    {/* Compact Chart */}
                    <div className="h-12 w-full">
                      <MiniChart data={section.data} type={section.id === "stores" ? "progress" : "bar"} />
                    </div>
                  </CardContent>
                </Card>
              </SheetTrigger>
            ...
            <SheetContent side="bottom" className="h-[80vh] rounded-t-xl">
              ...
              <div className="mt-6 space-y-4 overflow-y-auto h-full pb-6">
                {section.id === "trends" && <div className="space-y-4">
                    {analyticsData.priceChanges.map((period, index) => <Card key={index} className="p-4">
                ...
                {section.id === "categories" && <div className="space-y-3">
                    {analyticsData.categoryAnalysis.map((category, index) => <Card key={index} className="p-4">
                ...
                {section.id === "patterns" && <div className="space-y-3">
                    {analyticsData.pricePatterns.map((pattern, index) => <Card key={index} className="p-4">
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
