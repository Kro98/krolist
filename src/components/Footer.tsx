  ...
  return <footer className="w-full border-t border-border bg-background/80 backdrop-blur-sm mt-auto">
      <div className="container my-0 mx-[5px] px-[5px] py-[5px]">
        ...
        <div className="items-center gap-4 sm:items-center sm:justify-center sm:gap-[5px] mx-0 sm:flex sm:flex-row flex flex-row">
          {footerLinks.map((link, index) => <Link key={link.to} to={link.to} className="text-sm text-muted-foreground hover:text-foreground transition-colors px-0 mx-[5px] text-center">
              {link.label}
            </Link>)}
        </div>
