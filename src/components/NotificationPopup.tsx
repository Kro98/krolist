...
export function NotificationPopup() {
  ...
  return <div className="fixed top-0 left-0 right-0 z-[100] px-4 pt-4 pointer-events-none flex flex-col gap-2" style={{
    paddingTop: 'max(1rem, env(safe-area-inset-top))'
      ...
      return <div key={popup.notification.id} className={`max-w-md mx-auto w-full bg-card border border-border rounded-xl shadow-lg pointer-events-auto ${popup.isDragging ? '' : 'transition-all duration-300 ease-out'}`} style={{
        transform: `translateY(${popup.translateY}px)`,
        opacity: popup.opacity
      }} onTouchStart={e => handleTouchStart(e, popup.notification.id)} onTouchMove={e => handleTouchMove(e, popup.notification.id)} onTouchEnd={() => handleTouchEnd(popup.notification.id)}>
            {/* Swipe indicator */}
