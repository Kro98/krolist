    ...
    <>
      <div
        className={cn(
          ...
          isSelected ? "border-primary ring-2 ring-primary/30 scale-[0.98]" : "border-border/40",
      ...
      >
        ...
        <div className={cn("flex gap-3 p-3", isArabic && "flex-row-reverse")}>
          ...
          <div className="relative flex-shrink-0">
            ...
            {priceChange !== 0 && (
              <div className={cn(
                "absolute -bottom-1 -right-1 flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[10px] font-bold shadow-lg",
                priceChange < 0 
          ...
          <div className={cn("flex-1 min-w-0 flex flex-col justify-between", isArabic && "text-right")}>
            ...
            <div className={cn("flex items-start gap-2", isArabic && "flex-row-reverse")}>
              ...
              >
                <div ref={containerRef} className="title-scroll-container overflow-hidden">
                  <span
                    ref={titleRef}
                    className="title-scroll-text font-semibold text-sm leading-tight hover:text-primary transition-colors inline-block whitespace-nowrap"
              ...
              <div className={cn(
                ...
                isHovered || isSelectionMode ? "opacity-100" : "opacity-0 sm:opacity-100"
              )}>
                {onRemoveFromMyProducts && (
                  <Button
                    variant="ghost"
                    size="sm"
                    className="h-7 w-7 p-0 hover:bg-destructive/10 rounded-full transition-all active:scale-90"
                    onClick={(e) => {
                      e.stopPropagation();
            ...
            <div className={cn("flex items-center gap-1.5 mt-2 flex-wrap", isArabic && "flex-row-reverse")}>
              ...
              {product.category && (
                <Badge variant="outline" className="px-2 py-0.5 text-[10px] font-medium border-border/50 rounded-full">
                  {product.category}
                </Badge>
              )}
              {product.youtube_url && (
                <Button
                  size="sm"
                  variant="ghost"
                  className="h-5 w-5 p-0 hover:bg-red-500/10 rounded-full"
                  onClick={(e) => {
                    e.stopPropagation();
