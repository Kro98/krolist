...
serve(async (req) => {
  ...
  try {
    ...
    // Check if user has already refreshed this week (1 per week, non-accumulating)
    ...
    if (currentCount >= 1) {
      return new Response(
        JSON.stringify({ 
          ...
          message: 'You have already refreshed this week. Next refresh available on Sunday.',
